...
from diffusion_policy.env.aloha.constants import MASTER_GRIPPER_POSITION_NORMALIZE_FN
...
from diffusion_policy.env.aloha.env_utils import sample_box_rand_train_pose
import IPython
e = IPython.embed
...
def make_sim_env(task_name):
    ...
    if 'sim_transfer_cube' in task_name:
        ...
        xml_path = os.path.join(XML_DIR, f'bimanual_viperx_transfer_cube.xml')
        physics = mujoco.Physics.from_xml_path(xml_path)
        task = HistoryTransferCubeTask(random=False)
        env = control.Environment(physics, task, time_limit=20, control_timestep=DT,
                                  n_sub_steps=None, flat_observation=False)   
    elif "sim_pickandplace_samepos_scripted" in task_name:
        xml_path = os.path.join(XML_DIR, f'bimanual_viperx_transfer_cube.xml')
        physics = mujoco.Physics.from_xml_path(xml_path)
        task = PickAndPlaceAtSamePosCubeTask(random=False)
        env = control.Environment(physics, task, time_limit=20, control_timestep=DT,
                                  n_sub_steps=None, flat_observation=False)  
    elif "sim_pickandplace_twomodes_scripted" in task_name:
        xml_path = os.path.join(XML_DIR, f'bimanual_viperx_transfer_cube.xml')
        physics = mujoco.Physics.from_xml_path(xml_path)
        task = PickAndPlaceAtTwoModesCubeTask(random=False)
        env = control.Environment(physics, task, time_limit=20, control_timestep=DT,
                                  n_sub_steps=None, flat_observation=False)  
    elif "sim_singlearm_pickandplace_origin_scripted" in task_name:
        xml_path = os.path.join(XML_DIR, f'singlearm_viperx_transfer_cube.xml')
        physics = mujoco.Physics.from_xml_path(xml_path)
        task = SingleArmPickAndPlaceAtOriginCubeTask(random=False)
        env = control.Environment(physics, task, time_limit=20, control_timestep=DT,
                                  n_sub_steps=None, flat_observation=False)  
    elif "sim_singlearm_pickandplace_twomodes_scripted" in task_name:
        xml_path = os.path.join(XML_DIR, f'singlearm_viperx_transfer_cube.xml')
        physics = mujoco.Physics.from_xml_path(xml_path)
        task = SingleArmPickAndPlaceTwoModesCubeTask(random=False)
        env = control.Environment(physics, task, time_limit=20, control_timestep=DT,
                                  n_sub_steps=None, flat_observation=False)  
    elif "sim_pickandplace_origin_scripted" in task_name:
        xml_path = os.path.join(XML_DIR, f'bimanual_viperx_transfer_cube.xml')
        physics = mujoco.Physics.from_xml_path(xml_path)
        task = PickAndPlaceAtOriginCubeTask(random=False)
        env = control.Environment(physics, task, time_limit=20, control_timestep=DT,
                                  n_sub_steps=None, flat_observation=False)  
    elif 'sim_history_pickandplace_transfer_cube_scripted' in task_name:
        xml_path = os.path.join(XML_DIR, f'bimanual_viperx_transfer_cube.xml')
        physics = mujoco.Physics.from_xml_path(xml_path)
        task = HistoryPickAndPlaceTransferCubeTask(random=False)
...
class SingleArmViperXTask(base.Task):
    def __init__(self, random=None, camera_names=["top", "right_wrist"], hw=(84,84)):
    ...
    def get_observation(self, physics):
        ...
        for cam in self.camera_names:
            obs['images'][cam] = physics.render(height=self.hw[0], width=self.hw[1], camera_id=cam)
        return obs
...
class BimanualViperXTask(base.Task):
    def __init__(self, random=None, camera_names=["top", "left_wrist", "right_wrist"]):
        super().__init__(random=random)
        self.camera_names = camera_names
    ...
    def get_observation(self, physics):
        ...
        obs['images'] = dict() 
        for cam in self.camera_names:
            obs['images'][cam] = physics.render(height=84, width=84, camera_id=cam)
...
class PickAndPlaceAtOriginCubeTask(BimanualViperXTask):
    ...
    def initialize_episode(self, physics):
        ...
        self.cube_pose_target = np.array([0.1,0.5, 0.01])
    ...
    def get_reward(self, physics):
        ...
        curr_box_pos = self.get_observation(physics)["env_state"][:3]

        distance_to_target = np.linalg.norm(curr_box_pos - self.cube_pose_target)
        arm_rest = False
        reward = 0
...
class SingleArmPickAndPlaceAtOriginCubeTask(SingleArmViperXTask):
    def __init__(self, random=None):
        super().__init__(random=random)
        self.max_reward = 4
        self.rand_type = "rand_train"
    ...
    def initialize_episode(self, physics):
        ...
        # reset qpos, control and box position
        with physics.reset_context():
            physics.named.data.qpos[:8] = START_ARM_POSE[8:16]
            np.copyto(physics.data.ctrl, START_ARM_POSE[8:16])
            box_pose = sample_box_rand_train_pose()
            # box_pose = BOX_POSE[0]
...
class SingleArmPickAndPlaceTwoModesCubeTask(SingleArmViperXTask):
    def __init__(self, random=None):
        super().__init__(random=random)
        self.max_reward = 4
        self.rand_type = "rand_train"
    ...
    def initialize_episode(self, physics):
        ...
        self.cube_pose_target = get_target_pose(box_pose[:3])
...
class PickAndPlaceAtTwoModesCubeTask(BimanualViperXTask):
    def __init__(self, random=None):
        super().__init__(random=random)
        self.max_reward = 4
    ...
    def initialize_episode(self, physics):
        ...
        self.touched_table = 0
        from diffusion_policy.env.aloha.ee_sim_env import get_target_pose
        self.cube_pose_start = get_target_pose(box_pose[:3])
        self.reached_rest = False
    ...
    def get_reward(self, physics):
        ...
        distance_to_target = np.linalg.norm(curr_box_pos - self.cube_pose_start)
        arm_rest = False
        reward = 0
        if touch_right_gripper:
            reward = 1
...
class PickAndPlaceAtSamePosCubeTask(BimanualViperXTask):
    def __init__(self, random=None):
        super().__init__(random=random)
        self.max_reward = 4
    ...
    def initialize_episode(self, physics):
        ...
        self.t = 0
        self.touched_table = 0
        self.cube_pose_start = box_pose[:3]
        self.reached_rest = False
...
class HistoryPickAndPlaceTransferCubeTask(BimanualViperXTask):
    def __init__(self, random=None):
        super().__init__(random=random)
        self.max_reward = 8
...
class HistoryTransferCubeTask(BimanualViperXTask):
    def __init__(self, random=None):
        super().__init__(random=random)
        self.max_reward = 5
