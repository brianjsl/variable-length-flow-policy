...
class NoiseGenerator:
    ...
    def step(self, pred):
        ...
        noise_step = np.random.randn(*pred.shape) * self.noise_strength * pred
        # noise_step = pred * (np.random.rand(pred.shape[0], 1, pred.shape[2]) + 0.5) * np.random.choice([-1, 1], size=(pred.shape[0], 1, pred.shape[2])) * self.noise_strength
