...
class RobomimicImagePolicy(BaseImagePolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        import IPython
        IPython.embed()
        robomimic_obs_dict = dict_apply(nobs_dict, lambda x: x[:,:To,...])
    ...
    def train_on_batch(self, batch, epoch, validate=False):
        ...
        if not self.past_action_pred:
            info["losses"]["log_probs"] = torch.mean(info["predictions"]["log_probs"][:, [-1]])
            info["losses"]["action_loss"] = -torch.mean(info["predictions"]["log_probs"][:, [-1]])
